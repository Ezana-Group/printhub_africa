...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      mockups: p.mockups.map((m: any) => ({ platform: m.platform, imageUrl: m.imageUrl, isApproved: m.isApproved })),
      videos: p.videos.map((v: any) => ({ platform: v.platform, videoUrl: v.videoUrl, isApproved: v.isApproved })),
      captions,
