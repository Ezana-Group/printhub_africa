...
export async function GET(req: Request) {
  ...
  try {
    ...
    const status = searchParams.get("status") || ""; // 'error', 'success', 'waiting'

    const query = new URLSearchParams({ limit });
    if (status) query.set("status", status);

    const response = await fetch(`${n8nUrl}/api/v1/executions?${query.toString()}`, {
      headers: {
    ...
    console.error("[n8n Executions Proxy Error]:", error);
    return NextResponse.json({ error: "Failed to fetch executions from n8n" }, { status: 500 });
  }
}
