...
**Database protection:** Seed and clean scripts only run if `PRINTHUB_SEED_ALLOW=1` is set in `.env`/`.env.local` and the connected database is the PrintHub DB (has the `DatabaseGuard` row from PrintHub migrations). This prevents accidentally seeding or cleaning another project's database when you have multiple similar projects.

```bash
npm run db:clean-test-data
# or: npx tsx prisma/scripts/clean-test-data.ts
```

- [ ] Script run; orders, quotes, test users (except super admin), test products/catalogue, support tickets, reviews, coupons, job listings cleaned
- [ ] Only super admin (`admin@printhub.africa`) remains
- [ ] Business costs (LF), legal pages, FAQs, and email templates re-seeded automatically
