...
export const customerAuthOptions: NextAuthOptions = {
  ...
  cookies: {
    sessionToken: {
      name: process.env.NODE_ENV === "production" ? "__Host-ph-customer" : "ph-customer-dev",
      options: {
        httpOnly: true,
        sameSite: "lax",
        path: "/",
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      if (dbUser && (dbUser.role === "STAFF" || dbUser.role === "ADMIN" || dbUser.role === "SUPER_ADMIN")) {
        // Reject admin roles from the customer storefront
