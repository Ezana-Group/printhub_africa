...
export function CatalogueItemFormFull({ item, categories }: CatalogueItemFormFullProps) {
  ...
  const onCataloguePhotoComplete = async (results: UploadedFileResult[]) => {
    ...
    try {
      const res = await fetch(`/api/admin/catalogue/${item.id}/photos`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileIds: results.map((r) => r.uploadId) }),
      });
      const data = await res.json();
      if (res.ok && data.photos?.length) {
        const newPhotos = data.photos.map((p: { id: string; url: string; isPrimary: boolean; sortOrder: number }) => ({
  ...
  const onCatalogueStlComplete = async (results: UploadedFileResult[]) => {
    ...
    try {
      const res = await fetch(`/api/admin/catalogue/${item.id}/stl`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ uploadId: first.uploadId }),
      });
      const data = await res.json();
      if (res.ok) {
        setStlFileName(data.stlFileName ?? first.originalName);
        setStlFileSizeBytes(data.stlFileSizeBytes ?? first.sizeBytes);
      }
    } finally {
          ...
          <CardContent>
            ...
            <div className="mt-3">
              <FileUploader
                ...
                label="Upload your print photos"
                description="JPEG, PNG, WebP · Max 20MB · Upload YOUR prints — not designer renders"
                onUploadComplete={onCataloguePhotoComplete}
                onUploadError={(msg) => setError(msg)}
                disabled={photoUploading || photos.length >= 5}
              />
            </div>
            <p className="text-xs text-gray-400 mt-1">Or use file picker:</p>
            <Button
              variant="outline"
          ...
          <CardContent>
            ...
            <div className="mt-3">
              <FileUploader
                ...
                accept={["model/stl", "model/obj", "application/octet-stream"]}
                maxSizeMB={200}
                maxFiles={1}
                label={(stlFileName ?? item.stlFileName) ? "Replace STL file" : "Upload STL / 3D model"}
                description="STL, OBJ, 3MF · Max 200MB · Private"
                onUploadComplete={onCatalogueStlComplete}
                onUploadError={(msg) => setError(msg)}
                disabled={stlUploading}
              />
            </div>
            <p className="text-xs text-gray-400 mt-1">Or choose file:</p>
            <Button
              variant="outline"
              className="w-full mt-1"
              size="sm"
