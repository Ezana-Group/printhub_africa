...
type OrderItem = {
  ...
  catalogueItem?: { name: string; sku?: string | null; photos?: { url: string }[] } | null;
};
            ...
            <CardContent>
              {order.items.map((item) => {
                ...
                const imageUrl =
                  ...
                  item.catalogueItem?.photos?.[0]?.url;
                const productName = item.product?.name ?? item.catalogueItem?.name ?? "Custom";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(item.product?.sku ?? item.productVariant?.sku ?? item.catalogueItem?.sku) && (
                        <p className="text-xs font-mono text-gray-400">
                          SKU: {item.product?.sku ?? item.productVariant?.sku ?? item.catalogueItem?.sku}
                        </p>
                      )}
                    </div>
                ...
                <div className="space-y-2 pb-3 border-b">
                  ...
                  {order.status === "QUALITY_CHECK" && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => updateStatus("READY_FOR_COLLECTION")}
                    >
