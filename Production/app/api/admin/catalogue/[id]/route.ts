...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const item = await prisma.catalogueItem.findUnique({ where: { id }, select: { status: true } });
    if (!item) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (item.status === CatalogueStatus.DRAFT || item.status === CatalogueStatus.RETIRED) {
    ...
    if ((e as { code?: string })?.code === "P2025") return NextResponse.json({ error: "Not found" }, { status: 404 });
    console.error("Delete/retire catalogue item error:", e);
