...
export default function GetAQuotePage() {
  ...
  const [referenceUploadIds, setReferenceUploadIds] = useState<string[]>([]);
  ...
  const onUploadComplete = (results: UploadedFileResult[]) => {
    setUploadedFileIds(results.map((r) => r.uploadId));
  };

  const onReferenceUploadComplete = (results: UploadedFileResult[]) => {
    setReferenceUploadIds(results.map((r) => r.uploadId));
  };
    ...
    try {
      ...
      const res = await fetch("/api/quotes", {
        ...
        body: JSON.stringify({
          ...
          uploadIds: referenceUploadIds.length > 0 ? referenceUploadIds : undefined,
          specifications: {
    ...
    try {
      const res = await fetch("/api/quotes", {
        ...
        body: JSON.stringify({
          ...
          uploadIds: uploadedFileIds.length > 0 ? uploadedFileIds : undefined,
          specifications: {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                      ...
                      <div className="p-6 space-y-5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-0.5 mb-2">Logos, mood boards, or any reference files (optional).</p>
                          <FileUploader
                            context="CUSTOMER_QUOTE"
                            accept={["image/jpeg", "image/png", "image/webp", "application/pdf", "application/octet-stream", "application/zip"]}
                            maxSizeMB={200}
                            maxFiles={5}
                            label="Drop images or click to browse"
                            description="JPG, PNG, WebP, PDF · 5 max · 200MB each"
                            onUploadComplete={onReferenceUploadComplete}
                        ...
                        <div>
                          ...
                          {hasBrandLogo === true && (
                            <div className="mt-2">
                              <FileUploader
                                context="CUSTOMER_QUOTE"
                                accept={["image/png", "image/jpeg", "image/svg+xml", "application/pdf", "application/postscript"]}
                                maxSizeMB={50}
                                maxFiles={1}
                                label="Upload logo file (optional)"
                                description="PNG, JPG, SVG, PDF, AI · 50MB"
                                onUploadComplete={(results) => setReferenceUploadIds((prev) => [...prev, ...results.map((r) => r.uploadId)])}
                                onUploadError={(msg) => setErrorMsg(msg)}
                              />
                            </div>
                          )}
                    ...
                    <div className="space-y-6 lg:sticky lg:top-6">
                      <Card className="rounded-2xl overflow-hidden">
                        <CardHeader className="pb-2">
                          ...
                          <CardDescription>Files upload directly to our storage. Max {maxFiles} files, {maxSizeMb}MB each.</CardDescription>
                        </CardHeader>
                        <CardContent className="p-4 pt-0">
                          <FileUploader
                            context={serviceType === "large_format" ? "CUSTOMER_LARGE_FORMAT" : "CUSTOMER_3D_PRINT"}
                            accept={
                              serviceType === "large_format"
                                ? ["application/pdf", "image/png", "image/jpeg", "image/tiff", "image/svg+xml", "application/postscript", "image/vnd.adobe.photoshop", "application/dxf"]
                                : ["model/stl", "model/obj", "application/octet-stream", "application/sla"]
                            }
                            maxSizeMB={maxSizeMb}
                            maxFiles={maxFiles}
                            label="Drag and drop or click to browse"
                            description={serviceType === "large_format" ? "AI, PDF, PSD, EPS, PNG, JPG, SVG" : "STL, OBJ, 3MF, STEP"}
                            onUploadComplete={onUploadComplete}
                            onUploadError={(msg) => { setErrorMsg(msg); setStatus("error"); }}
                          />
                        </CardContent>
                      </Card>
                      {serviceType === "3d_print" ? (
              ...
              <section>
                ...
                <form onSubmit={handleUploadSubmit} className="space-y-6" suppressHydrationWarning>
                  ...
                  {uploadedFileIds.length > 0 && (
                    <p className="text-xs text-slate-500">{uploadedFileIds.length} file(s) attached.</p>
                  )}
                </form>
              </section>
            </>
